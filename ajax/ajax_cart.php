...
$giasize = (isset($_POST['giasize']) && $_POST['giasize'] > 0) ? htmlspecialchars($_POST['giasize']) : 0;
// $giasizecu = (isset($_POST['giasizecu']) && $_POST['giasizecu'] > 0) ? htmlspecialchars($_POST['giasizecu']) : 0;

if ($cmd == 'add-cart' && $id > 0) {
	$cart->addtocart($quantity, $id, $mau, $size, $giasize);
	$giatong += $gia;
	$max = (isset($_SESSION['cart'])) ? count($_SESSION['cart']) : 0;
	$data = array('max' => $max, 'giatong' => $giatong);
	...
	if (isset($_SESSION['cart'])) {
		...
		for ($i = 0; $i < $max; $i++) {
			if ($code == $_SESSION['cart'][$i]['code']) {
				...
				if ($gia) $_SESSION['cart'][$i]['giasize'] = $giasize;
				break;
			}
		}
	}
		...
		<div class="wrap-cart">
			<div class="top-cart">
				...
				<div class="modal-footer d-flex align-items-center justify-content-between">
					...
					<a class="btn-cart btn btn-primary" href="gio-hang?step=giohang" title="<?= thanhtoan ?>"><?= thanhtoan ?></a>
				</div>
			</div>
		</div>
	</form>
