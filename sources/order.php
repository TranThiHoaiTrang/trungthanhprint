...
if (isset($_POST['thanhtoan'])) {
	...
	$htttText = (isset($_POST['payments']) && $_POST['payments'] != '') ? htmlspecialchars($_POST['payments']) : '';
	...
	for ($i = 0; $i < $max; $i++) {
		...
		if ($giasize) {
			$chitietdonhang .= '<td align="left" style="padding:3px 9px" valign="top">';
			$chitietdonhang .= '<span style="display:block;color:#ff0000;">' . $func->format_money($giasize) . '</span>';
		...
		if ($giasize) {
			...
			$chitietdonhang .= '<span style="display:block;color:#ff0000;">' . $func->format_money($giasize * $q) . '</span>';
	...
	$data_donhang['httt_text'] = $htttText;
	...
	if ($id_insert) {
		for ($i = 0; $i < $max; $i++) {
			...
			$code = $_SESSION['cart'][$i]['code'];
			$giasize = $_SESSION['cart'][$i]['giasize'];
			...
			$data_donhangchitiet['giasize'] = $giasize;
	...
	/* Xóa giỏ hàng */
	unset($_SESSION['cart']);
	$func->transfer("Thông tin đơn hàng đã được gửi thành công.", $config_base . 'gio-hang?step=hoantat&madonhang=' . $madonhang);
}
