...
if (isset($_POST['submit-contact'])) {
	...
	if (($scoreCaptcha >= 0.5 && $actionCaptcha == 'contact') || $testCaptcha == true) {
		...
		if (!empty($emailer->getEmail('emailnguoigui'))) {
			if ($emailer->sendEmail("admin", $arrayEmail, $subject, $message, $file)) {
				...
				// if ($emailer->sendEmail("customer", $arrayEmail, $subject, $message, $file)) $func->dktc($config_base);
				// $func->transfer("Gửi thông tin thành công!",$config_base);
				$func->dktc($config_base);
...
$lienhe = $d->rawQueryOne("select * from #_static where type = ? limit 0,1", array('lienhe'));
