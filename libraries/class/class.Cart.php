<?php
	class Cart
	{
		...
		public function get_order_total()
		{
			...
			if(isset($_SESSION['cart']))
			{
				...
				for($i=0;$i<$max;$i++)
				{
					...
					if($giasize) $price = $giasize;
					elseif($proinfo['giamoi']) $price = $proinfo['giamoi'];
		...
		public function addtocart($q=1, $pid=0, $mau=0, $size=0, $giasize=0)
		{
			...
			if(isset($_SESSION['cart']))
			{
				if(!$this->product_exists($code,$q))
				{
					...
					$_SESSION['cart'][$max]['giasize'] = $giasize;
				}
			}
			else
			{
				...
				$_SESSION['cart'][0]['giasize'] = $giasize;
			}
		}
