...
class Functions
{
	...
	{
		$row = $this->d->rawQuery("select * from #_rating where id_product = ? and hienthi > 0", array($id));
		$ratingall = $this->d->rawQuery("select * from #_rating where id_product = ? and hienthi > 0", array($id));

		$rating_tong = 0;
		$tongrating = 0;
		foreach ($ratingall as $v) {
			$tongrating += $v['rating'];
		}
		$rating_tong = round($tongrating / count($ratingall), 1);

		if ($ratingall) {
			$return = '<span>' . count($ratingall) . '</span>';
			// $return = '<span>Kết quả '.$rating_tong.'/5 ('.count($ratingall).' đánh giá)</span>';
		} else {
			$return = '<span>0 </span>';
			// $return = '<span>Kết quả 0/5 (0 đánh giá)</span>';
		}

		return $return;
	}
	public function get_rating_number($id = 0)
	{
		$row = $this->d->rawQuery("select * from #_rating where id_product = ? and hienthi > 0", array($id));
		$ratingall = $this->d->rawQuery("select * from #_rating where id_product = ? and hienthi > 0", array($id));
		...
		if ($ratingall) {
			// $return = '<span>'.count($ratingall).' đánh giá</span>';
			$return = $rating_tong;
		} else {
			// $return = '<span>0 đánh giá</span>';
			$return = '0';
		}

		return $return;
	}
	public function get_phantram_rating($id = 0)
	{
		if ($id) {
			$row = $this->d->rawQuery("select * from #_rating where id_product = ? and hienthi > 0", array($id));
	...
	public function get_phantram_rating_people($id = 0)
	{
		if ($id) {
			$row = $this->d->rawQuery("select * from #_rating where id = '" . $id . "' and hienthi > 0");
			// var_dump($row);
			$rating_tong = 0;
			$tongrating = 0;
			foreach ($row as $v) {
	...
	public function get_phantram_rating_loai($id = 0, $rating = 0)
	{
		if ($id) {
			$row = $this->d->rawQuery("select * from #_rating where id_product = '" . $id . "' and rating = '" . $rating . "' and hienthi > 0");
			$countong_rating = $this->d->rawQueryOne("select COUNT(*) as count FROM #_rating where id_product = '" . $id . "' and hienthi > 0");
			// var_dump("select rating, COUNT(*) as count FROM #_rating where id_product = '".$id."' GROUP BY rating");
			if (!empty($row)) {
				$rating_tong = 0;
				$tongrating = 0;
				// foreach($row as $v){
				...
				// }
				// $rating_tong = round($tongrating / count($row),1);
				$rating_tong = count($row);

				$phantranrating = round($rating_tong * 10);
				// var_dump(($phantranrating * 10));
				// var_dump($countong_rating['count']);
				// var_dump(($phantranrating * 10)/5);
				return (($phantranrating * 10) / $countong_rating['count']);
			} else {
				return 0;
			}
		} else {
	...
	public function dktc($page = '', $stt = true)
	{
		global $config_base;
		$stt = $stt;
		$page_transfer = $page;
		include("./templates/layout/dktc.php");
		exit();
	}
	...
	public function pagination($totalq = 0, $per_page = 10, $page = 1, $url = '?')
	{
		...
		if ($lastpage > 1) {
			...
			if ($page > 1) {
				...
				$pagination .= "<li class='page-item'><a class='page-link'>{$prevlabel}</a></li>";
			}
			...
			if ($page < $counter - 1) {
				...
				$pagination .= "<li class='page-item'><a class='page-link'>{$nextlabel}</a></li>";
			}
	...
	/* Get category by link */
	public function get_link_danhmuc_cap($table = '', $level = '', $type = '', $title_select = 'Chọn danh mục')
	{
		$where = '';
		$params = array($type);
		$id_parent = 'id_' . $level;

		if ($level == 'danhmuc_cap') {
			// $iddanhmuc = (isset($_REQUEST['id_danhmuc'])) ? htmlspecialchars($_REQUEST['id_danhmuc']) : 0;
			// $where .= ' and id_danhmuc = ?';
			// array_push($params, $iddanhmuc);
			$title_select = 'Chọn danh mục cấp';
		}

		$rows = $this->d->rawQuery("select tenvi, id from #_" . $table . "_" . $level . " where type = ? " . $where . " order by stt,id desc", $params);
		// var_dump("select tenvi, id from #_".$table."_".$level." where type = ? ".$where." order by stt,id desc",$params);
		$str = '<select id="' . $id_parent . '" name="' . $id_parent . '" onchange="onchange_danhmuccap($(this))" class="form-control filer-danhmuccap select2"><option value="0">' . $title_select . '</option>';
		foreach ($rows as $v) {
			if (isset($_REQUEST[$id_parent]) && ($v["id"] == (int)$_REQUEST[$id_parent])) $selected = "selected";
			else $selected = "";
