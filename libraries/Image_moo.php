 ...
 * @author WEBHD
 */
final class Image_moo
{
    // image vars
    protected $main_image;
    protected $watermark_image;
    protected $temp_image;
    protected int $jpeg_quality = 75;
    protected string $background_colour = "#ffffff";
    protected $watermark_method;
    protected bool $jpeg_ignore_warnings = false;    // set to true or call ignore_jpeg_warnings()
    protected bool $can_stretch = false; // when a resizing an image too small, allow it to be stretched larger

    // other
    protected string $filename = "";

    // watermark stuff, opacity
    protected int $watermark_transparency = 50;
    ...
    {
        if ($this->jpeg_ignore_warnings) $this->ignore_jpeg_warnings(true);
        if ($this->can_stretch) $this->allow_scale_up(true);
    }

    /**
     * having loaded lots of jpegs I quite often get corrupt ones, this setting relaxes GD a bit
     * requires 5.1.3 php
     *
     * @param bool $onoff
     * @return $this
     */
    public function ignore_jpeg_warnings(bool $onoff = true)
    {
    ...
    /**
     * If you want to stretch or crop images that are smaller than the target size, call this with TRUE to scale up
     *
     * @param bool $onoff
     * @return $this
     */
    public function allow_scale_up(bool $onoff = false)
    {
    ...
    /**
     * returns the errors formatted as needed
     *
     * @param $open
     * @param $close
     * @return string
     ...
     * @return bool
     */
    public function is_gd_image($image): bool
    {
     ...
     * @return bool
     */
    private function _check_image(): bool
    {
        // generic check
    ...
    /**
     * Saves image as a data-stream for inline inclusion
     ...
     */
    public function get_data_stream(string $filename = "")
    {
     ...
     * @return $this
     */
    public function save_dynamic(string $filename = "")
    {
        // validate we loaded a main image
        if (!$this->_check_image()) return $this;
     ...
     * @return $this
     */
    public function save_pa(string $prepend = "", string $append = "", bool $overwrite = false)
    {
        // validate we loaded a main image
        if (!$this->_check_image()) return $this;
     ...
     * @return $this
     */
    public function save($filename, bool $overwrite = false)
    {
        // validate we loaded a main image
        if (!$this->_check_image()) return $this;
     ...
     * @return string
     */
    public function real_filesize(): string
    {
        // filename?
        if ($this->filename == "") {
     ...
     * @return $this
     */
    public function set_watermark_transparency(int $transparency = 50)
    {
     ...
     * @return $this
     */
    public function set_background_colour(string $colour = "#ffffff")
    {
     ...
     * @return $this
     */
    public function set_jpeg_quality(int $quality = 75)
    {
     ...
     * @return void
     */
    private function _copy_to_temp_if_needed(): void
    {
        if (!$this->is_gd_image($this->temp_image)) {
    ...
    /**
     * take main image and resize to temp image using boundaries mw,mh (max width or max height)
    ...
    public function stretch($mw, $mh)
    {
        ...
        // copy resized (stetted, proportions not kept);
     ...
     * text is the text to write, $fontfile is a ttf file that will be used $size=font size, $colour is the colour of text
     *
     * @param $text
     * @param $fontfile
     * @param $size
    ...
    /**
     * add a solid-border  frame, coloured $colour to the image
     *
     * @param $width
     * @param $colour
     * @return $this
    ...
    public function border_3d($width = 5, $rot = 0, $opacity = 30)
    {
        ...
        switch ($rot) {
            case 1 :
                $cols = [$white, $black, $white, $black];
                break;
            case 2 :
                $cols = [$black, $black, $white, $white];
                break;
            case 3 :
                $cols = [$black, $white, $black, $white];
                break;
            default :
                $cols = [$white, $white, $black, $black];
        }
    ...
    public function shadow($size = 4, $direction = 3, $colour = "#444")
    {
        ...
        switch ($direction) {
            ...
            default:
                $sh_x = $size;
                $pic_x = 0;
                $this->set_error("Shadow position not in valid range 7,8,9 - 4,5,6 - 1,2,3");
        }
        // do y position
        switch ($direction) {
            // y top
    ...
    /**
     * allows you to use the in-bulit gd2 image filters
     *
     * @param $function
     * @param $arg1
     * @param $arg2
     ...
     * using a quarter and rotating as you can end up with odd rounding if you draw a whole and use parts
     *
     * @param $radius
     * @param $invert
     * @param $corners
    ...
    public function round($radius = 5, $invert = false, $corners = "")
    {
        ...
        // check input
        if ($corners == "") $corners = [true, true, true, true];
    ...
    /**
     * Updates the new_width and height sizes
     *
     * @return void
     */
