<?php
	...
	if(isset($_POST["listemail"]) && isset($_POST['tieude']) && isset($_POST['noidung']))
	{
		...
		for($i=0;$i<count($listemail);$i++)
		{
			...
			$row = $d->rawQueryOne("select id, email, ten from #_newsletter where id = ? and type = ? limit 0,1",array($id,'dangkinhantin'));
	...
	function get_items()
	{
		...
		$items = $d->rawQuery($sql,array('dangkinhantin'));
	...
	function get_item()
	{
		...
		$item = $d->rawQueryOne("select * from #_newsletter where id = ? and type = ? limit 0,1",array($id,'dangkinhantin'));
		...
		if($id)
		{
			if(isset($_FILES['file-taptin']))
			{
				...
				if($taptin = $func->uploadImage("file-taptin", $config['newsletter'][$type]['file_type'],UPLOAD_FILE,$file_name))
				{
					...
					$row = $d->rawQueryOne("select id, taptin from #_newsletter where id = ? and type = ? limit 0,1",array($id,'dangkinhantin'));
	...
	function delete_item()
	{
		...
		if($id)
		{
			...
			if($row['id'])
			{
				...
				// var_dump("delete from table_newsletter where id = '$id' and type = '$type'");die();
				$d->rawQuery("delete from #_newsletter where id = ? and type = ?",array($id,'dangkinhantin'));
