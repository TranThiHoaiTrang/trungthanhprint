...
require dirname(__DIR__, 3) . DIRECTORY_SEPARATOR . 'bootstrap.php';
...
const ELFINDER_IMAGEMAGICK_PS = true;
// ===============================================

// // load composer autoload before load elFinder autoload If you need composer
// // You need to run the composer command in the php directory.
is_readable(dirname(__DIR__, 1) . '/vendor/autoload.php') && require dirname(__DIR__, 1) . '/vendor/autoload.php';
...
const ELFINDER_DISABLE_ZIPEDITOR = false; // set `true` to disable zip editor
 ...
 * @return bool|null
 **/
function access($attr, $path, $data, $volume, $isDir, $relpath): ?bool
{
    // set locked all of folder
 ...
 * @return string
 */
function sanitize_filename($filename, $opts): string
{
    $_file_ext = Helper::fileExtension($filename, false);
    $_file_name = Helper::fileName($filename, false);
    $_file_name = Helper::url_title($_file_name, $opts['replace'], true);

    if ( $_file_ext ) {
        return str_replace($opts['targets'], $opts['replace'], $_file_name) . '.' . $_file_ext;
    }

    return str_replace($opts['targets'], $opts['replace'], $_file_name);
}

/**
 * @return string[]
 */
function getDisabled(): array
{
    return [ 'netmount', 'tmb', 'mkfile', 'chmod', 'callback', 'rename', 'duplicate', 'empty', 'hide' ];
}
...
$_path = UPLOADS_PATH . $current_year . DIRECTORY_SEPARATOR . $current_month . DIRECTORY_SEPARATOR;
$_URL = UPLOADS_URL . $current_year . '/' . $current_month . '/';

// Create dirs
Helper::create_directory($_path);
Helper::create_directory(UPLOADS_PATH . '.trash/');
Helper::create_directory(UPLOADS_PATH . '.tmb/');
...
$opts = [
    ...
    'roots' => [
        ...
        [
            ...
            'path' => UPLOADS_PATH,                  // path to files (REQUIRED)
            'URL' => UPLOADS_URL,               // URL to files (REQUIRED)
            ...
            //'disabled' => ['netmount', 'tmb', 'mkdir', 'mkfile', 'chmod', 'callback', 'rename', 'duplicate', 'empty', 'hide'],
        ...
        [
            'id' => '1',
            'driver' => 'Trash',
            'path' => UPLOADS_PATH . '.trash' . DIRECTORY_SEPARATOR,
            'tmbURL' => UPLOADS_URL . '.tmb/',
            'winHashFix' => DIRECTORY_SEPARATOR !== '/', // to make hash same to Linux one on windows too
