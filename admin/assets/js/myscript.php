<script>
	...
	/* onChange Category */
	function filter_danhmuccap(url) {
		if ($(".filer-danhmuccap").length > 0 && url != '') {
			var id = '';
			var value = 0;

			$(".filer-danhmuccap").each(function() {
				id = $(this).attr("id");
				if (id) {
					value = parseInt($("#" + id).val());
					if (value) {
	...
	function onchange_danhmuccap(obj) {
		var name = '';
		var keyword = $("#keyword").val();
		var url = "<?= (isset($linkMan) && $linkMan != '') ? $linkMan : '' ?>";

		obj.parents(".form-group").nextAll().each(function() {
			name = $(this).find(".filer-danhmuccap").attr("name");
			if ($(this) != obj) {
				$(this).find(".filer-danhmuccap").val(0);
			}
		});

		url = filter_danhmuccap(url);
	...
	/* canonical */
	function canonicalConvert(canonical, focus = false) {
		canonical = canonical.toLowerCase();
		canonical = canonical.replace(/á|à|ả|ạ|ã|ă|ắ|ằ|ẳ|ẵ|ặ|â|ấ|ầ|ẩ|ẫ|ậ/gi, 'a');
		canonical = canonical.replace(/é|è|ẻ|ẽ|ẹ|ê|ế|ề|ể|ễ|ệ/gi, 'e');
		canonical = canonical.replace(/i|í|ì|ỉ|ĩ|ị/gi, 'i');
		canonical = canonical.replace(/ó|ò|ỏ|õ|ọ|ô|ố|ồ|ổ|ỗ|ộ|ơ|ớ|ờ|ở|ỡ|ợ/gi, 'o');
		canonical = canonical.replace(/ú|ù|ủ|ũ|ụ|ư|ứ|ừ|ử|ữ|ự/gi, 'u');
		canonical = canonical.replace(/ý|ỳ|ỷ|ỹ|ỵ/gi, 'y');
		canonical = canonical.replace(/đ/gi, 'd');
		canonical = canonical.replace(/\`|\~|\!|\@|\#|\||\$|\%|\^|\&|\*|\(|\)|\+|\=|\,|\.|\/|\?|\>|\<|\'|\"|\:|\;|_/gi, '');
		canonical = canonical.replace(/ /gi, "-");
		canonical = canonical.replace(/\-\-\-\-\-/gi, '-');
		canonical = canonical.replace(/\-\-\-\-/gi, '-');
		canonical = canonical.replace(/\-\-\-/gi, '-');
		canonical = canonical.replace(/\-\-/gi, '-');
	...
	function canonicalPreview(title, lang, focus = false) {
		var canonical = canonicalConvert(title, focus);

		$("#canonical" + lang).val(canonical);
		$("#canonicalurlpreview" + lang + " strong").html(canonical);
	}

	function canonicalPreviewTitleSeo(title, lang) {
	...
	function canonicalPress() {
		var canonicallang = "vi,en";
		var inputArticle = $('.card-article input.for-seo');
		var id = $('.canonical-id').val();
		var seourlstatic = true;
		//var seourlstatic = $(".canonical-seo-preview").data("seourlstatic");
		inputArticle.each(function(index) {
			var ten = $(this).attr('id');
			var lang = ten.substr(ten.length - 2);
			if (canonicallang.indexOf(lang) >= 0) {
				if ($("#" + ten).length) {
					$('body').on("keyup", "#" + ten, function() {
						var title = $("#" + ten).val();

						if ((!id || $("#canonicalchange").prop("checked")) && seourlstatic) {
							/* canonical preivew */
							canonicalPreview(title, lang);
						}

						/* canonical preivew title seo */
						canonicalPreviewTitleSeo(title, lang);

						/* canonical Alert */
						canonicalAlert(2, lang);
					})
				}

				if ($("#canonical" + lang).length) {
					$('body').on("keyup", "#canonical" + lang, function() {
						var title = $("#canonical" + lang).val();

						/* canonical preivew */
						canonicalPreview(title, lang, true);
	...
	function canonicalChange(obj) {
		if (obj.is(':checked')) {
			/* Load canonical theo tiêu đề mới */
			canonicalStatus(1);
			$(".canonical-input").attr("readonly", true);
		} else {
			/* Load canonical theo tiêu đề cũ */
			canonicalStatus(0);
			$(".canonical-input").attr("readonly", false);
		}
	}
		...
		inputArticle.each(function(index) {
			...
			if (canonicallang.indexOf(lang) >= 0) {
				...
				if (status == 1) {
					...
					if ($("#canonical-default" + lang).length) {
						title = $("#canonical-default" + lang).val();
	...
	function canonicalAlert(result, lang) {
		if (result == 1) {
			$("#alert-canonical-danger" + lang).addClass("d-none");
			$("#alert-canonical-success" + lang).removeClass("d-none");
		} else if (result == 0) {
			$("#alert-canonical-danger" + lang).removeClass("d-none");
			$("#alert-canonical-success" + lang).addClass("d-none");
		} else if (result == 2) {
			$("#alert-canonical-danger" + lang).addClass("d-none");
			$("#alert-canonical-success" + lang).addClass("d-none");
		}
	...
	function canonicalCheck() {
		var canonicallang = "vi,en";
		var canonicalInput = $('.canonical-input');
		var id = $('.slug-id').val();
		var copy = $('.slug-copy').val();

		canonicalInput.each(function(index) {
			var canonicalId = $(this).attr('id');
			var canonical = $(this).val();
			var lang = canonicalId.substr(canonicalId.length - 2);
			if (canonicallang.indexOf(lang) >= 0) {
				if (canonical) {
					$.ajax({
						url: 'ajax/ajax_canonical.php',
						type: 'POST',
						dataType: 'html',
						async: false,
						data: {
	...
	$(document).ready(function() {
		...
		$('body').on("click", ".create-faq", function() {
			var soLuongTheDiv = parseInt($('.count_faq').val());
			var soluong = soLuongTheDiv + 1;
			$('.count_faq').val(soluong);
			$('.tab_pane_faq').append('<div class="all_faq_content"><div class="form-group"><div class="label-seo"><label for="titlevi">FAQ title '+soluong+' (vi):</label></div><input type="text" class="form-control check-seo title-seo" name="datafaq' + soluong + '[tenvi]" id="tenvi" placeholder="FAQ Title ' + soluong + '(vi)" value=""></div><div class="form-group"><div class="label-seo"><label for="noidungvi">FAQ Description '+soluong+' (vi):</label></div><textarea class="form-control check-seo description-seo ckeditor" name="datafaq' + soluong + '[noidungvi]" id="noidungvi" rows="5" placeholder="FAQ Description (vi)"></textarea></div></div>')
