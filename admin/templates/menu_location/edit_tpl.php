    ...
    <div class="container-fluid">
        <div class="row">
            <ol class="breadcrumb float-sm-left">
                ...
                <li class="breadcrumb-item active">Sửa vị trí menu</li>
            </ol>
        </div>
    </div>
</section>
                                        ...
                                        <?php foreach($config['website']['lang'] as $k => $v) : ?>
                                            <div class="tab-pane fade show <?=($k=='vi')?'active':''?>" id="tabs-lang-<?=$k?>" role="tabpanel" aria-labelledby="tabs-lang">
                                                ...
                                                <div class="form-group">
                                                    ...
                                                    <input type="text" class="form-control" name="data[ten<?=$k?>]" id="ten<?=$k?>" placeholder="Tiêu đề (<?=$k?>)" value="<?=@$item['ten'.$k]?>" <?=($k=='vi')?'required':''?>>
                                                </div>

                                                <div class="form-group">
                                                    <label for="stt" class="d-inline-block align-middle mb-0 mr-2">Số thứ tự:</label>
                                                    <input type="number" class="form-control form-control-mini d-inline-block align-middle" name="data[stt]" id="stt" placeholder="Số thứ tự" value="<?= $item['stt'] ?? 0 ?>">
                                                </div>
                                                        ...
                                                        foreach ( $_locations as $i => $loc ) :
                                                            ...
                                                            foreach ($items as $item_loc) {
                                                                ...
                                                                if ( $item_loc['location'] ) {
                                                                    ...
                                                                    if ( in_array( $i, $tmp) ) $_disabled = ' disabled';
                                                                }
                                                            ...
                                                            <div class="form-row">
                                                                <div class="custom-control custom-checkbox d-inline-block align-middle">
                                                                    <input<?=$_disabled . $_checked?> type="checkbox" class="custom-control-input" name="location[]" id="<?=$i?>" value="<?=$i?>">
