                                ...
                                <div class="tab-pane fade show <?=($k=='vi')?'active':''?>" id="tabs-lang-<?=$k?>" role="tabpanel" aria-labelledby="tabs-lang">
                                    ...
                                    <?php if(isset($config['tags'][$type]['mota']) && $config['tags'][$type]['mota'] == true) { ?>
                                        <div class="form-group">
                                            ...
                                            <textarea class="form-control for-seo <?=(isset($config['tags'][$type]['mota_cke']) && $config['tags'][$type]['mota_cke'] == true)?'ckeditor':''?>" name="data[mota<?=$k?>]" id="mota<?=$k?>" rows="5" placeholder="Mô tả (<?=$k?>)"><?=htmlspecialchars_decode(@$item['mota'.$k])?></textarea>
                                        </div>
                                    <?php } ?>
                                    <?php if(isset($config['tags'][$type]['noidung']) && $config['tags'][$type]['noidung'] == true) { ?>
                                        <div class="form-group">
                                            <label for="noidung<?=$k?>">Nội dung (<?=$k?>):</label>
                                            <textarea class="form-control for-seo <?=(isset($config['tags'][$type]['noidung_cke']) && $config['tags'][$type]['noidung_cke'] == true)?'ckeditor':''?>" name="data[noidung<?=$k?>]" id="noidung<?=$k?>" rows="5" placeholder="Nội dung (<?=$k?>)"><?=htmlspecialchars_decode(@$item['noidung'.$k])?></textarea>
                                        </div>
                                    <?php } ?>
                                </div>
                            <?php } ?>
