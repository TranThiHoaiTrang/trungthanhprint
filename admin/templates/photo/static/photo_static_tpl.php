            ...
            <div class="card-body">
                <?php if(isset($config['photo']['photo_static'][$type]['images']) && $config['photo']['photo_static'][$type]['images'] == true) { ?>
                    <div class="form-group">
                        <?php if($type == 'popup') {?>
                        <h3 class="card-title" style="float: none;margin-bottom: 1.5rem;">Hình ảnh tìm gia sư</h3>
                        <?php }else{ ?>
                            <h3 class="card-title" style="float: none;margin-bottom: 1.5rem;">Hình ảnh</h3>
                        <?php } ?>
                        <div class="static-body">
                            <?php
                            $photoDetail = @$item['photo'] ?? '';
                            $dimension = "(".$config['photo']['photo_static'][$type]['img_type'].")";

                            $table_name = 'photo';
                            $input_name = $table_key = 'photo';
                            include LAYOUT . "static_image.php";
                            ?>
                        </div>
                    </div>
                <?php } ?>
                <?php if(isset($config['photo']['photo_static'][$type]['photo1']) && $config['photo']['photo_static'][$type]['photo1'] == true) { ?>
                    <div class="form-group">
                    <?php if($type == 'popup') {?>
                        <h3 class="card-title" style="float: none;margin-bottom: 1.5rem;">Hình ảnh tìm lớp dạy</h3>
                        <?php }else{ ?>
                            <h3 class="card-title" style="float: none;margin-bottom: 1.5rem;">Hình ảnh</h3>
                        <?php } ?>
                        <div class="static-body">
                ...
                <div class="row">
                    ...
                    <?php } ?>
                    <?php if(isset($config['photo']['photo_static'][$type]['link1']) && $config['photo']['photo_static'][$type]['link1'] == true) { ?>
                        <div class="form-group col-md-6">
                            <label for="link1">link tìm lớp dạy:</label>
                            <input type="text" class="form-control" name="data[link1]" id="link1" placeholder="link tìm lớp dạy" value="<?=@$item['link1']?>">
                        </div>
                    <?php } ?>
                                    ...
                                    <div class="tab-pane fade show <?=($k=='vi')?'active':''?>" id="tabs-lang-<?=$k?>" role="tabpanel" aria-labelledby="tabs-lang">
                                        ...
                                        <?php if((isset($config['photo']['photo_static'][$type]['mota']) && $config['photo']['photo_static'][$type]['mota'] == true)) { ?>
                                            <div class="form-group">
                                                ...
                                                <textarea class="form-control <?=((isset($config['photo']['photo_static'][$type]['mota_cke']) && $config['photo']['photo_static'][$type]['mota_cke'] == true))?'ckeditor':''?>" name="data[mota<?=$k?>]" id="mota<?=$k?>" rows="5" placeholder="Mô tả (<?=$k?>)"><?=htmlspecialchars_decode(@$item['mota'.$k])?></textarea>
                                            </div>
                                        <?php } ?>
                                        <?php if((isset($config['photo']['photo_static'][$type]['noidung']) && $config['photo']['photo_static'][$type]['noidung'] == true)) { ?>
                                            <div class="form-group">
                                                <label for="noidung<?=$k?>">Nội dung (<?=$k?>):</label>
                                                <textarea class="form-control <?=((isset($config['photo']['photo_static'][$type]['noidung_cke']) && $config['photo']['photo_static'][$type]['noidung_cke'] == true))?'ckeditor':''?>" name="data[noidung<?=$k?>]" id="noidung<?=$k?>" rows="5" placeholder="Nội dung (<?=$k?>)"><?=htmlspecialchars_decode(@$item['noidung'.$k])?></textarea>
                                            </div>
                                        <?php } ?>
                                    </div>
                                <?php } ?>
