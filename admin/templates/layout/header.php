<?php
    ...
    $dangkyNotify = $d->rawQuery("select id from #_newsletter where hienthi = 0 and type='dangkinhantin' ");
    $dangkyTuvan = $d->rawQuery("select id from #_contact where hienthi = 0 and type='dktv' ");
    $dangkyDaily = $d->rawQuery("select id from #_contact where hienthi = 0 and type='dkdl' ");

    $countNotify += count($contactNotify) + count($dangkyNotify) + count( $dangkyTuvan); + count( $dangkyDaily);
    ...
    <ul class="navbar-nav ml-auto">
        ...
        <li class="nav-item dropdown">
            ...
            <div class="dropdown-menu dropdown-menu-right shadow">
                ...
                <a href="index.php?com=contact&act=man&type=dktv" class="dropdown-item"><i class="fas fa-envelope mr-2"></i><span class="badge badge-danger mr-1"><?=count($dangkyTuvan)?></span> Đăng kí tư vấn</a>
                <a href="index.php?com=contact&act=man&type=dkdl" class="dropdown-item"><i class="fas fa-envelope mr-2"></i><span class="badge badge-danger mr-1"><?=count($dangkyDaily)?></span> Đăng kí đại lý</a>
                ...
                <?php if(isset($config['newsletter'])) { ?>
                    ...
                    <?php foreach($config['newsletter'] as $k => $v) { 
                        $emailNotify = $d->rawQuery("select id from #_newsletter where type = ? and hienthi = 0",array($k)); ?>
                        <a href="index.php?com=newsletter&act=man&type=<?=$k?>" class="dropdown-item"><i class="fas fa-mail-bulk mr-2"></i></i><span class="badge badge-danger mr-1"><?=count($dangkyNotify)?></span> <?=$v['title_main']?></a>
                        <div class="dropdown-divider"></div>
                    <?php } ?>
                <?php } ?>
            </div>
